...
public class Taquin {
    ...
    public Taquin() {
        ...
        // Losowe wykonanie do 20 ruchów
    }

    // Funkcja do wykonywania losowych ruchów
    public void makeRandomMoves(int maxMoves) {
        Random rand = new Random();
        int moveCount = 0;
    
        // Generowanie losowych ruchów, maksymalnie 20 ruchów
        while (moveCount < maxMoves) {
            ArrayList<Taquin> neighbors = getNeighbors();
            if (neighbors.isEmpty()) break;  // Jeśli brak sąsiadów, kończymy
            
            Taquin randomNeighbor = neighbors.get(rand.nextInt(neighbors.size()));
            
            // Zamiast tworzyć nowy obiekt Taquin, aktualizujemy stan bieżącego obiektu
            this.arr = randomNeighbor.arr.clone();  // Przypisujemy losowy stan do układanki
            this.moves = randomNeighbor.moves;      // Aktualizujemy liczbę ruchów
            this.history = new ArrayList<>(randomNeighbor.history);  // Kopiujemy historię
